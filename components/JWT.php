...
class JWT
{
    ...
    public static function encodeShort(array $payload, $key)
    {
        ksort($payload);
        $params = http_build_query($payload);
        $segments = [];
        $segments[] = JWT::urlsafeB64Encode($params);
        $segments[] = substr(md5("$key$params"), 0, 16);
        return implode('.', $segments);
    }

    public static function decodeShort($jwt, $key, $verify = true)
    {
        $tks = explode('.', $jwt);
        if (count($tks) != 2) {
            throw new UnexpectedValueException('Wrong number of segments', 1);
        }
        list($payloadb64, $sign) = $tks;
        if (null === ($payloadParams = JWT::urlsafeB64Decode($payloadb64))) {
            throw new UnexpectedValueException('Invalid segment encoding', 1);
        }
        $beSign = substr(md5("$key$payloadParams"), 0, 16);
        if ($sign != $beSign) {
            throw new UnexpectedValueException('Signature verification failed', 2);
        }
        parse_str($payloadParams, $payload);
        if ($verify) {
            // 签发时间大于当前服务器时间验证失败
            if (isset($payload['iat']) && $payload['iat'] > time()) {
                throw new UnexpectedValueException('Token iat is not valid before: ' . date(DateTime::ISO8601, $payload['iat']), 3);
